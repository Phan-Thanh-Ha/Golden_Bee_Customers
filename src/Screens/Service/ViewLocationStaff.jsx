...
const ViewLocationStaff = () => {
  ...
  console.log("data booking on view staff screen: ", data);
    ...
    if (data?.StaffId && data?.CustomerId) {
      const unsubscribe = OVG_FBRT_ListenOrderUpdate(
        ...
        (order) => {
          ...
          if (order?.LatitudeStaff && order?.LongitudeStaff) {
            setStaffLocation({
              latitude: order?.LatitudeStaff,
              longitude: order?.LongitudeStaff,
            });
          }
        },
        (orderId) => {
          console.log(`Order ${orderId} đã bị xóa`);
          setClientOrder(null);
    ...
    <SafeAreaView style={styles.container}>
      <View>
        ...
        >
          ...
          {staffLocation && (
            <MapViewDirections
              ...
              destination={{
                latitude: data?.LatitudeStaff,
                longitude: data?.LongitudeStaff,
              }}
        ...
        <LayoutPosition style={{ top: 10, left: 10, right: 10 }}>
          <CardLocation
            onPress={() => { }}
            location={data?.DataService?.Address}
          />
        </LayoutPosition>
      </View>
      <ScrollView>
